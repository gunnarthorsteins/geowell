...
class Distance:
    ...
    def synthesize_values(self, i_proposed: int, x_to_interpolate: float, y_to_interpolate: float):
        ...
        print(g)
        x_interp = interpolate(a=self.proposed_well[i_proposed-1:i_proposed+1, -1],
                               b=x_to_interpolate)
        y_interp = interpolate(a=self.proposed_well[i_proposed-1:i_proposed+1, -1],
                               b=y_to_interpolate)
    ...
    def calculate_distance(self, x_interp, y_interp, i_proposed):
        # Calculate distance
        distance = np.sqrt((x_interp-self.proposed_well[i_proposed, 0])**2 + \
    ...
    def run(self):
        ...
        for _, well in self.incumbent_wells.iterrows():
            ...
            z_incumbent = np.linspace(, well['MaxFDypi'], len(self.proposed_well))
            for i_proposed in range(1, len(self.proposed_well)):
                for k in range(1, len(z_incumbent) - 1):
                    if z_incumbent[k] < self.proposed_well[i_proposed, 2] < z_incumbent[k+1]:
                        x_interp, y_interp = self.synthesize_values(i_proposed=i_proposed,
                                                                    x_to_interpolate=np.repeat(well['x'], 2),
                                                                    y_to_interpolate=np.repeat(well['y'], 2))
                        well_distance_temp.append(self.calculate_distance(x_interp, y_interp))
