...
class GUI:
    ...
    def _2d_annotation(self):
        ...
        cell_text = list(zip(settings["default_values"].keys(),
                             settings["default_values"].values(),
                             settings["units"]))
        col_labels = ['param', 'val', 'unit']
        table = self.ax_2d.table(cellText=cell_text,
    ...
    def plot_3d_trajectory(self, x: np.array, y: np.array, z: np.array, i: int):
        ...
        self.ax_3d.plot(y[i-1:], x[i-1:], z[i-1:], c=settings['palette']['red'], solid_capstyle='round')  # i-1 to ensure overlap w. casing
    ...
    def plot_incumbent_wells(self, wells: pd.DataFrame):
        ...
        for j in range(len(x)):
            ...
            self.ax_3d.plot(y_linspace, x_linspace, z_linspace, c='k', solid_capstyle='round')
